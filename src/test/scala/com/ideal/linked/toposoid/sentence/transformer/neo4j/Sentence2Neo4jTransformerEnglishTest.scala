...
class Sentence2Neo4jTransformerEnglishTest extends AnyFlatSpec with BeforeAndAfter with BeforeAndAfterAll {
  ...
  "The list of english sentences" should "be properly registered in the knowledge database and searchable." in {
    ...
    val result: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'That'})-[:LocalEdge]->(:ClaimNode{surface:"\\'s"})<-[:LocalEdge]-(:ClaimNode{surface:'life'}) RETURN x""", transversalState)
    assert(result.records.size == 1)
    val result2: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'Seeing'})-[:LocalEdge]->(:ClaimNode{surface:'believing'})<-[:LocalEdge]-(:ClaimNode{surface:'is'}) RETURN x""", transversalState)
    assert(result2.records.size == 1)
    val result3: Neo4jRecords = TestUtils.executeQueryAndReturn("""MAtCH x = (:SynonymNode{nodeName:'living'})-[:SynonymEdge]->(:ClaimNode{surface:'life'}) return x""", transversalState)
    assert(result3.records.size == 1)
    val result4: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"That's life."}) RETURN x""", transversalState)
    assert(result4.records.size == 1)
    val result5: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:'Seeing is believing.'}) RETURN x""", transversalState)
    assert(result5.records.size == 1)
    val result6: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"That's life."})-[*]-(:SemiGlobalClaimNode{sentence:'Seeing is believing.'}) RETURN x""", transversalState)
  ...
  "The list of multiple english sentences" should "be properly registered in the knowledge database and searchable." in {
    ...
    val result2:Neo4jRecords =TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'Seeing'})-[:LocalEdge]->(:ClaimNode{surface:'believing'})<-[:LocalEdge]-(:ClaimNode{surface:'is'}) RETURN x""",  transversalState)
    assert(result2.records.size == 1)
    val result3: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"That\\'s life. Seeing is believing."}) RETURN x""",  transversalState)
    assert(result3.records.size == 1)
  }
  ...
  "The List of english sentences including a premise" should "be properly registered in the knowledge database and searchable." in {
    ...
    val knowledgeSentenceSetForParser = KnowledgeSentenceSetForParser(List.empty[KnowledgeForParser], List.empty[PropositionRelation], knowledgeList, List.empty[PropositionRelation])
    Sentence2Neo4jTransformer.createGraph(getAnalyzedPropositionSet(knowledgeSentenceSetForParser, transversalState), transversalState)
    val result:Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode)-[*..]->(:ClaimNode{surface:'dream'})-[:LocalEdge]->(:ClaimNode{surface:'do'})<-[*..]-(:ClaimNode) RETURN x""",  transversalState)
    assert(result.records.size >= 1)
    val result2: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:'If you can dream it, you can do it.'}) RETURN x""",  transversalState)
    assert(result2.records.size == 1)
  ...
  "The list of english sentences with json" should "be properly registered in the knowledge database and searchable." in {
    ...
    //assert(result2.records.size == 1)
    val result3: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"That\\'s life."})-[:SemiGlobalEdge{logicType:'AND'}]-(:SemiGlobalClaimNode{sentence:'Seeing is believing.'}) RETURN x""",  transversalState)
    assert(result3.records.size == 1)
  ...
  "The short english sentence with json" should "be properly registered in the knowledge database and searchable." in {
    ...
    //assert(!result3.records.size == 1)
    val result4: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:'nature'}) RETURN x""",  transversalState)
    assert(result4.records.size == 2)
    val result5: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:'nature'}) RETURN x""",  transversalState)
  ...
  "The List of Japanese Premises and empty Claims" should "be properly registered in the knowledge database and searchable." in {
    ...
    Sentence2Neo4jTransformer.createGraph(getAnalyzedPropositionSet(knowledgeSetForParser, transversalState), transversalState)
    val result: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:PremiseNode{surface:'A'})-[*]->(:PremiseNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'AND'}]->(:PremiseNode{surface:'is',isDenialWord:'true'})<-[*]-(:PremiseNode{surface:'B'}) RETURN x""",  transversalState)
    assert(result.records.size == 1)
    val result2: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:PremiseNode{surface:'B'})-[*]->(:PremiseNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'OR'}]->(:PremiseNode{surface:'is',isDenialWord:'true'})<-[*]-(:PremiseNode{surface:'C'}) RETURN x""",  transversalState)
    assert(result2.records.size == 1)
    val result3: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalPremiseNode{sentence: "A\\'s hair is not black."})-[:SemiGlobalEdge{logicType:'AND'}]->(:SemiGlobalPremiseNode{sentence:"B\\'s hair is not blonde"})-[:SemiGlobalEdge{logicType:'OR'}]->(:SemiGlobalPremiseNode{sentence:"C\\'s hair is not black."}) RETURN x""",  transversalState)
    assert(result3.records.size == 1)
  ...
  "The List of English Claims and empty Premises" should "be properly registered in the knowledge database and searchable." in {
    ...
    Sentence2Neo4jTransformer.createGraph(getAnalyzedPropositionSet(knowledgeSetForParser, transversalState), transversalState)
    val result: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'A'})-[*]->(:ClaimNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'AND'}]->(:ClaimNode{surface:'is',isDenialWord:'true'})<-[*]-(:ClaimNode{surface:'B'}) RETURN x""",  transversalState)
    assert(result.records.size == 1)
    val result2:Neo4jRecords =TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'B'})-[*]->(:ClaimNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'OR'}]->(:ClaimNode{surface:'is',isDenialWord:'true'})<-[*]-(:ClaimNode{surface:'C'}) RETURN x""",  transversalState)
    assert(result2.records.size == 1)
    val result3: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"A\\'s hair is not black."})-[:SemiGlobalEdge{logicType:'AND'}]->(:SemiGlobalClaimNode{sentence:"B\\'s hair is not blonde"})-[:SemiGlobalEdge{logicType:'OR'}]->(:SemiGlobalClaimNode{sentence:"C\\'s hair is not black."}) RETURN x""",  transversalState)
    assert(result3.records.size == 1)
  ...
  "The List of Japanese Claims and Premises" should "be properly registered in the knowledge database and searchable." in {
    ...
    val result2:Neo4jRecords =TestUtils.executeQueryAndReturn("""MATCH x = (:PremiseNode{surface:'B'})-[*]->(:PremiseNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'OR'}]->(:PremiseNode{surface:'is',isDenialWord:'true'})<-[*]-(:PremiseNode{surface:'C'}) RETURN x""",  transversalState)
    assert(result2.records.size == 1)
    val result3:Neo4jRecords =TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'D'})-[*]->(:ClaimNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'OR'}]->(:ClaimNode{surface:'is',isDenialWord:'true'})<-[*]-(:ClaimNode{surface:'E'}) RETURN x""",  transversalState)
    assert(result3.records.size == 1)
    val result4:Neo4jRecords =TestUtils.executeQueryAndReturn("""MATCH x = (:ClaimNode{surface:'E'})-[*]->(:ClaimNode{surface:'is',isDenialWord:'true'})-[:LocalEdge{logicType:'AND'}]->(:ClaimNode{surface:'is',isDenialWord:'true'})<-[*]-(:ClaimNode{surface:'F'}) RETURN x""",  transversalState)
    assert(result4.records.size == 1)
    val result5: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:PremiseNode{surface:'A'})-[*]->(:PremiseNode{surface:'is'})-[:LocalEdge{logicType:'IMP'}]->(:ClaimNode{surface:'is'})<-[*]-(:ClaimNode{surface:'D'}) RETURN x""",  transversalState)
    assert(result5.records.size == 1)
    val result6: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalPremiseNode{sentence:"A\\'s hair is not black."})-[:SemiGlobalEdge{logicType:'AND'}]->(:SemiGlobalPremiseNode{sentence:"B\\'s hair is not blonde"})-[:SemiGlobalEdge{logicType:'OR'}]->(:SemiGlobalPremiseNode{sentence:"C\\'s hair is not black."}) RETURN x""",  transversalState)
    assert(result6.records.size == 1)
    val result7: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalClaimNode{sentence:"D\\'s hair is not black."})-[:SemiGlobalEdge{logicType:'OR'}]->(:SemiGlobalClaimNode{sentence:"E\\'s hair is not blonde"})-[:SemiGlobalEdge{logicType:'AND'}]->(:SemiGlobalClaimNode{sentence:"F\\'s hair is not black."}) RETURN x""",  transversalState)
    assert(result7.records.size == 1)
    val result8: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:SemiGlobalPremiseNode{sentence:"A\\'s hair is not black."})-[:SemiGlobalEdge{logicType:'IMP'}]-(:SemiGlobalClaimNode{sentence:"D\\'s hair is not black."}) RETURN x""",  transversalState)
