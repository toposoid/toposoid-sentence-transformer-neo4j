...
class Sentence2Neo4JTransformerDocumentEnglishTest extends AnyFlatSpec with BeforeAndAfter with BeforeAndAfterAll {
  ...
  "The list of global premise and claim in document" should "be properly registered in the knowledge database and searchable." in {
    ...
    val knowledgeClaimList = List(
      ...
      KnowledgeForParser(propositionId, UUID.random.toString, Knowledge("NO_REFFERENCE_" + documentId.toString + "_3", "en_US", "{}", false, knowledgeForDocument = knowledgeForDocument, documentPageReference = documentPageReference3)),
    )
    ...
    val result: Neo4jRecords = TestUtils.executeQueryAndReturn("""MATCH x = (:GlobalNode {titleOfTopPage: 'Test-Title', filename: 'test.pdf', url:'http://xxxx/test.pdf'}) RETURN x""", transversalState)
