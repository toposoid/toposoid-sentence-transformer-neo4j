...
class Sentence2Neo4JTransformerTableJapaneseTest extends AnyFlatSpec with BeforeAndAfter with BeforeAndAfterAll {
  ...
  "The list of local claim images" should "be properly registered in the knowledge database and searchable." in {
    ...
    val knowledgeList = List(
      KnowledgeForParser(java.util.UUID.randomUUID().toString, java.util.UUID.randomUUID().toString, Knowledge("これは表１です。", "ja_JP", "{}", false, List.empty[KnowledgeForImage], List(knowledgeForTable1))),
      KnowledgeForParser(java.util.UUID.randomUUID().toString, java.util.UUID.randomUUID().toString, Knowledge("これは表２です。", "ja_JP" ,"{}", false, List.empty[KnowledgeForImage], List(knowledgeForTable2)))
    )
